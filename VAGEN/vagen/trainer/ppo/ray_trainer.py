...
from vagen.mllm_agent.rollout_service_val import QwenVLValidationRolloutManagerService
...
def augment_with_next_value(data_proto: DataProto) -> DataProto:
    ...
    # 1.1 Print env_id and step_id for each sample before adding next_value
    # print("Before adding next_value:")
    # for sample_to_print in samples:
    #     print(f"Env ID: {sample_to_print['env_id']}, Step ID: {sample_to_print['step_id']}")
            ...
            if pos < len(idx_list) - 1:
                ...
                if isinstance(next_value, torch.Tensor):
                    if next_value.numel() == 1:
                        next_value = next_value.item()  # Keep as float, don't convert to int
                    else:
    ...
    # 2.1 Sort samples by step_id in descending order after adding next_value

    #### Necessary update for era: reverse the sorting order after the balancing batch function call
    samples.sort(key=lambda s: s['step_id'], reverse=True)

    # print("\nAfter adding next_value and sorting (descending by step_id):")
    # for sample_to_print in samples:
    #     print(f"Env ID: {sample_to_print['env_id']}, Step ID: {sample_to_print['step_id']}, Next Value: {sample_to_print.get('next_value', 'N/A')}")
...
def compute_advantage(data: DataProto, adv_estimator, gamma=1.0, lam=1.0, num_repeat=1,high_level_gamma=1.0):
    ...
    #### support multi-turn update with MASKED_GAE
        ...
        advantages, returns =core_algos.compute_gae_advantage_return_with_loss_mask(token_level_rewards=token_level_rewards,
                                                                values=values,
                                                                value_pos = data.non_tensor_batch['single_value_position'],
                                                                step_id=step_id,
                                                                env_id=env_id,
                                                                next_values=data.non_tensor_batch['next_value'],
                                                                loss_mask=gae_mask,
                                                                rewards=data.non_tensor_batch['single_reward'],
                                                                gamma=gamma,
                                                                lam=lam)
        data.batch['advantages'] = advantages
        data.batch['returns'] = returns
...
class RayPPOTrainer(object):
    ...
    def _validate(self, first_time=False):
        ...
        if self.test_rollout_manager==None:
            if self.config.rollout_manager.get("use_service",False):
                self.test_rollout_manager =QwenVLValidationRolloutManagerService(
        ...
        for batch_dict in self.val_dataloader:
            ...
            if hasattr(self.test_rollout_manager, 'save_conversation_history'):
                log_file_path = os.path.join(self.config.trainer.default_local_dir, f'conversation_history_val_step_{self.global_steps}_{substep}.txt')
                self.test_rollout_manager.save_conversation_history(self.global_steps, log_file_path)
    ...
    def fit(self):
        ...
        def get_filter_failure_count(global_step, critic_warmup_steps):
            """
            Calculate how many failure trajectories to filter at current step.
            - During warmup (steps 1-2): no filtering
            - After warmup: gradually reduce filtering from 16 to 0
            """
            if global_step <= critic_warmup_steps:
                return 0  # No filtering during warmup
        ...
        if self.config.rollout_manager.eval_only == True:
            self._validate()
            return
        ...
        for epoch in range(self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                ...
                # for memory to be released, another approach is to use offload
                # sleep(20)
                ...
                rollout_manager.reset(env_configs, has_val_first=self.config.trainer.val_before_train, global_steps=self.global_steps)
                ...
                with _timer('step', timing_raw):
                    ...
                    with _timer('gen', timing_raw):
                        ...
                        # Calculate filter count for curriculum learning
                        if self.config.trainer.curriculum_learning == 'enable':
                            filter_failure_count = get_filter_failure_count(self.global_steps, self.config.trainer.critic_warmup)
                            print(f"[Curriculum Learning] Global step {self.global_steps}: filtering {filter_failure_count} failure trajectories")
                        else:
                            filter_failure_count = 0
                        
                        final_gen_batch_output = rollout_manager.generate_batch_for_update(filter_failure_count=filter_failure_count)
                        rst= rollout_manager.recording_to_log() # data source == inputs in our current setting, outputs=whole 
                        
                        # 保存对话历史到txt文件
                        if hasattr(rollout_manager, 'save_conversation_history'):
                            log_file_path = os.path.join(self.config.trainer.default_local_dir, f'conversation_history_step_{self.global_steps}.txt')
                            rollout_manager.save_conversation_history(self.global_steps, log_file_path)
                    ...
                    with _timer('adv', timing_raw):
                        ...
                        #### modified for era: logging advantage/value/reward

                        advantages = batch.batch['advantages']  # (batch_size, seq_len)
                        values = batch.batch['values'] if 'values' in batch.batch.keys() else None  # (batch_size, seq_len)
                        single_value_positions = batch.non_tensor_batch['single_value_position']  # list of indices
                        step_ids = batch.non_tensor_batch['step_id']  # list of step_ids
                        env_ids = batch.non_tensor_batch['env_id']  # list of env_ids
                        rewards = batch.non_tensor_batch['single_reward']  # list of rewards
                        
                        # organize data by env_id
                        env_data = defaultdict(list)
                        
                        for i in range(len(env_ids)):
                            env_id = env_ids[i]
                            step_id = step_ids[i]
                            pos = single_value_positions[i]
                            reward = rewards[i]
                            
                            # extract the advantage value at the corresponding position
                            advantage_val = advantages[i, pos].item() if pos < advantages.shape[1] else 0.0
                            
                            # extract the value value at the corresponding position (if exists)
                            value_val = values[i, pos].item() if values is not None and pos < values.shape[1] else 0.0
                            
                            env_data[env_id].append({
                                'step_id': step_id,
                        ...
                        # sort data by step_id for each env
                        for env_id in env_data:
                            env_data[env_id].sort(key=lambda x: x['step_id'])
                        
                        # log to file
                        log_file_path = os.path.join(self.config.trainer.default_local_dir, 'advantage_value_log.txt')
                        os.makedirs(os.path.dirname(log_file_path), exist_ok=True)
                        
                        with open(log_file_path, 'a', encoding='utf-8') as f:
                            f.write(f"\n=== Global Step: {self.global_steps} ===\n")
                            for env_id, env_steps in env_data.items():
                                f.write(f"Env ID: {env_id}\n")
                                for step_data in env_steps:
                                    f.write(f"  Step {step_data['step_id']}: advantage={step_data['advantage']:.6f}, "
                                           f"value={step_data['value']:.6f}, reward={step_data['reward']:.6f}\n")
                                f.write("\n")
                    ...
                    if self.val_reward_fn is not None and self.config.trainer.test_freq > 0 and \
                        self.global_steps % self.config.trainer.test_freq == 0 and self.global_steps >= self.config.trainer.test_start:
                        with _timer('testing', timing_raw):
                            val_metrics: dict = self._validate()
                        metrics.update(val_metrics)

                    if self.config.trainer.save_freq > 0 and \
                            self.global_steps % self.config.trainer.save_freq == 0 and self.global_steps >= self.config.trainer.save_start:
