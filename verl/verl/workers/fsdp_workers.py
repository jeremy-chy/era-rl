...
class ActorRolloutRefWorker(Worker):
    ...
    def _build_model_optimizer(self,
        ...
        with init_context(), warnings.catch_warnings():
            ...
            if self.config.actor.freeze_vision_tower:
                for p in actor_module.visual.parameters():
                    p.requires_grad = False
                for p in actor_module.visual.merger.parameters():
        ...
        actor_module_fsdp = FSDP(
            ...
            use_orig_params=True if self.config.actor.freeze_vision_tower else False,
    ...
    @register(dispatch_mode=Dispatch.ONE_TO_ALL)
    def save_checkpoint(self, local_path, hdfs_path, global_step=0, max_ckpt_to_keep=1):
    ...
    @register(dispatch_mode=Dispatch.ONE_TO_ALL)
    def load_checkpoint(self, local_path, hdfs_path, del_local_after_load=False):
        if self._is_offload_param:
            load_fsdp_model_to_gpu(self.actor_module_fsdp)
...
class CriticWorker(Worker):
    ...
    def _build_critic_model_optimizer(self, config):
        ...
        with init_context(), warnings.catch_warnings():
            ...
            if "Qwen2.5-VL" in local_path or "era" in local_path or "Qwen" in local_path:
