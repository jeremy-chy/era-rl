...
class DataParallelPPOCritic(BasePPOCritic):
    ...
    def update_critic(self, data: DataProto):
        ...
        if self.config.use_reward_mask:
            select_keys = ['input_ids', 'responses', 'attention_mask', 'position_ids', 'values', 'returns','end_of_response_position_mask', 'loss_mask_for_critic']
        elif 'loss_mask' in data.batch.keys():
            select_keys = ['input_ids', 'responses', 'attention_mask', 'position_ids', 'values', 'returns','loss_mask', 'loss_mask_for_critic']
        else:
            select_keys = ['input_ids', 'responses', 'attention_mask', 'position_ids', 'values', 'returns', 'loss_mask_for_critic']
        batch = data.select(batch_keys=select_keys).batch
        has_multi_modal_inputs = 'multi_modal_inputs' in data.non_tensor_batch.keys()
            ...
            for batch_idx, data in enumerate(dataloader):
                ...
                for data in micro_batches:
                    ...
                    if self.config.use_reward_mask:
                        ...
                        loss_mask = data['loss_mask_for_critic'][:, -response_length:]
