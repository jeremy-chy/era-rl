...
def compute_gae_advantage_return_with_loss_mask(token_level_rewards: torch.Tensor, values: torch.Tensor,value_pos: torch.Tensor,
                                 step_id: list[int],env_id: list[int],
                                 next_values: torch.Tensor, loss_mask: torch.Tensor, rewards: torch.Tensor, gamma: float, lam: float):
    ...
    with torch.no_grad():
        ...
        for b in range(batch_size):
            ...
            if len(valid_positions) == 0:
                raise ValueError(f"No valid positions found for batch {b}")
            ...
            delta = rewards[b] + gamma * next_value - value
            gae = delta + gamma * lam * next_step_gae
            env_gae_dict[(curr_env_id, curr_step_id)] = gae

            for pos in valid_positions:
                advantages[b, pos] = gae
